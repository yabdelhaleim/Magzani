...
use App\Http\Controllers\MaterialDispensingController;
...
use App\Http\Controllers\Purchase\PurchaseInvoicePriceAdjustmentController;
use App\Http\Controllers\PurchaseInvoiceController;
use App\Http\Controllers\PurchaseReturnController;
use App\Http\Controllers\RawMaterialTemplateController;
use App\Http\Controllers\ReportingController;
use App\Http\Controllers\Reports\BatchTraceabilityReportController;
use App\Http\Controllers\Reports\CostVarianceReportController;
...
use App\Livewire\PosReturnPanel;
...
Route::middleware([
    ...
    */
    Route::middleware(['auth', 'feature:pos'])->group(function () {
        Route::get('/pos', PosPanel::class)->name('pos.index');
        Route::get('/pos/returns', PosReturnPanel::class)->name('pos.returns');
        Route::get('/pos/settings', [PosSettingController::class, 'index'])->name('pos.settings.index')->middleware('admin.only');
        Route::post('/pos/settings', [PosSettingController::class, 'update'])->name('pos.settings.update')->middleware('admin.only');
    ...
    */
    Route::middleware(['auth', 'feature:manufacturing'])->group(function () {
        ...
        Route::prefix('manufacturing')->name('manufacturing.')->middleware('admin.only')->group(function () {
            ...
            Route::get('material-batches', [MaterialBatchController::class, 'index'])->name('material-batches.index');
            Route::get('material-batches/create', [MaterialBatchController::class, 'create'])->name('material-batches.create');
            Route::post('material-batches', [MaterialBatchController::class, 'store'])->name('material-batches.store');
            Route::get('material-dispensings', [MaterialDispensingController::class, 'index'])->name('material-dispensings.index');
            Route::get('material-batches/{batch}/dispense', [MaterialDispensingController::class, 'create'])->name('material-dispensings.create');
            Route::post('material-dispensings', [MaterialDispensingController::class, 'store'])->name('material-dispensings.store');
            Route::get('material-dispensings/{dispensing}/create-invoice', [MaterialDispensingController::class, 'createInvoice'])->name('material-dispensings.create-invoice');
        ...
        Route::prefix('manufacturing-orders')->name('manufacturing-orders.')->middleware('admin.only')->group(function () {
            ...
            Route::post('/', [ManufacturingOrderController::class, 'store'])->name('store')->middleware('throttle:30,1');
            ...
            Route::match(['post', 'patch'], '/{manufacturingOrder}/confirm', [ManufacturingOrderController::class, 'confirm'])->name('confirm')->middleware('throttle:10,1');
            Route::match(['post', 'patch'], '/{manufacturingOrder}/complete', [ManufacturingOrderController::class, 'complete'])->name('complete')->middleware('throttle:10,1');
            Route::match(['post', 'patch'], '/{manufacturingOrder}/cancel', [ManufacturingOrderController::class, 'cancel'])->name('cancel')->middleware('throttle:10,1');
        });
    });
    ...
    */
    Route::middleware(['auth', 'feature:multi_warehouse'])->group(function () {
        Route::prefix('transfers')->name('transfers.')->group(function () {
            ...
            Route::post('/', [TransferController::class, 'store'])->name('store')->middleware('throttle:30,1');
            Route::get('/pending', [TransferController::class, 'pending'])->name('pending');
            Route::get('/{transfer}', [TransferController::class, 'show'])->name('show');
            Route::post('/{transfer}/reverse', [TransferController::class, 'reverse'])->name('reverse')->middleware('throttle:10,1');
            Route::post('/{transfer}/cancel', [TransferController::class, 'cancel'])->name('cancel')->middleware('throttle:10,1');
    ...
    */
    Route::middleware(['auth', 'feature:accounting'])->group(function () {
        ...
        Route::prefix('reports')->name('reports.')->middleware('admin.only')->group(function () {
            ...
            Route::get('/cost-variance', [CostVarianceReportController::class, 'index'])
                ->name('cost-variance');
            Route::get('/cost-variance/export', [CostVarianceReportController::class, 'exportCsv'])
                ->name('cost-variance.export');

            // Gap 4 — Batch Traceability Report
            Route::get('/batch-traceability', [BatchTraceabilityReportController::class, 'index'])
    ...
    */
    Route::prefix('purchase/invoices/{invoice}/items/{item}')->name('purchase.price-adjustment.')->group(function () {
        Route::get('/price-adjustment', [PurchaseInvoicePriceAdjustmentController::class, 'show'])
            ->name('show');
        Route::post('/price-adjustment', [PurchaseInvoicePriceAdjustmentController::class, 'apply'])
            ->name('apply');
    });
    ...
    */
    Route::prefix('warehouse-orders')->name('warehouse-orders.')->middleware(['auth', 'role', 'feature:warehouses'])->group(function () {
        Route::get('/stock-preview', [WarehouseOrderController::class, 'warehouseStockPreview'])->name('stock-preview');

        // Inbound Orders
        Route::get('/inbound', [WarehouseOrderController::class, 'inboundIndex'])->name('inbound.index');
        Route::get('/inbound/create', [WarehouseOrderController::class, 'inboundCreate'])->name('inbound.create');
        Route::post('/inbound', [WarehouseOrderController::class, 'inboundStore'])->name('inbound.store');
        Route::get('/inbound/{order}', [WarehouseOrderController::class, 'inboundShow'])->name('inbound.show');
        Route::get('/inbound/{order}/print', [WarehouseOrderController::class, 'inboundPrint'])->name('inbound.print');

        // Outbound Orders
        Route::get('/outbound', [WarehouseOrderController::class, 'outboundIndex'])->name('outbound.index');
        Route::get('/outbound/create', [WarehouseOrderController::class, 'outboundCreate'])->name('outbound.create');
        Route::post('/outbound', [WarehouseOrderController::class, 'outboundStore'])->name('outbound.store');
        Route::get('/outbound/{order}', [WarehouseOrderController::class, 'outboundShow'])->name('outbound.show');
        Route::get('/outbound/{order}/print', [WarehouseOrderController::class, 'outboundPrint'])->name('outbound.print');
        Route::post('/outbound/{order}/approve', [WarehouseOrderController::class, 'outboundApprove'])->name('outbound.approve');
        Route::post('/outbound/{order}/cancel', [WarehouseOrderController::class, 'outboundCancel'])->name('outbound.cancel');
    });
    ...
    */
    Route::prefix('categories')->name('categories.')->middleware('auth')->group(function () {
        ...
        Route::post('/', [CategoryController::class, 'store'])->middleware('throttle:30,1')->name('store');
        Route::put('/{category}', [CategoryController::class, 'update'])->middleware('throttle:30,1')->name('update');
        Route::delete('/{category}', [CategoryController::class, 'destroy'])->middleware('throttle:30,1')->name('destroy');
        Route::post('/{category}/toggle-status', [CategoryController::class, 'toggleStatus'])->middleware('throttle:30,1')->name('toggle-status');
    ...
    */
    Route::prefix('permissions')->name('permissions.')->middleware('auth', 'admin.only')->group(function () {
        ...
        Route::get('/users/{user}/edit', [PermissionsController::class, 'editUser'])->name('edit-user');
        Route::put('/users/{user}', [PermissionsController::class, 'updateUser'])->name('update-user');
        Route::get('/roles', [PermissionsController::class, 'roles'])->name('roles');
        Route::post('/roles', [PermissionsController::class, 'storeRole'])->name('store-role');
        Route::put('/roles/{role}', [PermissionsController::class, 'updateRole'])->name('update-role');
        Route::delete('/roles/{role}', [PermissionsController::class, 'destroyRole'])->name('destroy-role');
        Route::put('/roles/{role}/permissions', [PermissionsController::class, 'updateRolePermissions'])->name('update-role-permissions');
        Route::get('/print', [PermissionsController::class, 'printReport'])->name('print');
    });
