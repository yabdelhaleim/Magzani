...
@section('title', 'إضافة خامة جديدة')
@section('page-title', 'إضافة خامة جديدة')
    ...
    <form method="POST" action="{{ route('manufacturing-orders.raw-materials.store') }}">
        ...
        <div class="mfg-card">
            ...
            <div class="mfg-card-body">
                <p style="font-size:12px;color:var(--tf-text-m);margin:0 0 16px;line-height:1.5;">
                    بعد الحفظ تُنشأ صنف خام في المنتجات وتُربط كمية المخزن المختار — ستظهر الخامة في صفحة المخزن ضمن «خامات التصنيع».
                </p>
                <div class="form-group">
                    <label class="form-label">اسم الخامة</label>
                    <input type="text" name="name" class="form-control" required placeholder="مثال: فرش، روباط، شاسية، دكم" value="{{ old('name') }}">
                </div>
                ...
                <div class="grid-2">
                    <div class="form-group">
                        ...
                        <input type="number" name="quantity" class="form-control" required step="0.01" min="0" placeholder="0" value="{{ old('quantity', 0) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">سعر الشراء</label>
                        <input type="number" name="buy_price" class="form-control" required step="0.01" min="0" placeholder="0.00" value="{{ old('buy_price', 0) }}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">سعر البيع</label>
                        <input type="number" name="sale_price" class="form-control" required step="0.01" min="0" placeholder="0.00" value="{{ old('sale_price', 0) }}">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="action-buttons">
            <button type="submit" class="btn btn-green">
                <i class="fas fa-check"></i> حفظ الخامة
            </button>
