...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-purple-500 to-pink-500 px-6 py-4">
            <h2 class="text-2xl font-bold text-white">تعديل فاتورة شراء: {{ $invoice->invoice_number }}</h2>
        </div>

        <form method="POST" action="{{ route('invoices.purchases.update', $invoice->id) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg mb-6">
                <div class="flex items-start">
                    <i class="fas fa-exclamation-triangle text-yellow-600 ml-3 mt-1"></i>
                    <div>
                        <p class="font-medium">تنبيه!</p>
                        <p class="text-sm">عند تعديل الفاتورة، سيتم عكس الكميات القديمة من المخزن ثم إضافة الكميات الجديدة.</p>
                    </div>
                </div>
            </div>
