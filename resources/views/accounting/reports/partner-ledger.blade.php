...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100 font-sans">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">كشف حساب شريك تفصيلي (Partner Statement)</h2>
            <p class="text-gray-600 mt-1">كشف حركات تفصيلي ورصيد افتتاحي وختامي للعملاء والموردين</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 no-print font-sans">
        <form method="GET" action="{{ route('accounting.reports.partner-ledger') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">نوع الشريك</label>
                <select id="partner_type" name="partner_type" onchange="togglePartnerSelects()" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="customer" {{ $partnerType === 'customer' ? 'selected' : '' }}>عميل</option>
                    <option value="supplier" {{ $partnerType === 'supplier' ? 'selected' : '' }}>مورد</option>
                </select>
            </div>
            
            <div id="customer_select_wrapper" class="{{ $partnerType === 'customer' ? '' : 'hidden' }}">
                <label class="block text-sm font-medium text-gray-700 mb-2">العميل</label>
                <select name="partner_id" id="customer_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="">-- اختر العميل --</option>
                    @foreach($customers as $cust)
                        <option value="{{ $cust->id }}" {{ $partnerType === 'customer' && $partnerId == $cust->id ? 'selected' : '' }}>
                            {{ $cust->code }} - {{ $cust->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div id="supplier_select_wrapper" class="{{ $partnerType === 'supplier' ? '' : 'hidden' }}">
                <label class="block text-sm font-medium text-gray-700 mb-2">المورد</label>
                <select name="partner_id" id="supplier_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" disabled>
                    <option value="">-- اختر المورد --</option>
                    @foreach($suppliers as $supp)
                        <option value="{{ $supp->id }}" {{ $partnerType === 'supplier' && $partnerId == $supp->id ? 'selected' : '' }}>
                            {{ $supp->code }} - {{ $supp->name }}
                        </option>
                    @endforeach
                </select>
    ...
    @if($ledger)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden font-sans">
            ...
            <div class="p-6 bg-gray-50 border-b border-gray-100 text-center">
                <h3 class="text-xl font-bold text-gray-800">كشف حركات الحساب التفصيلي للشريك</h3>
                <p class="text-sm text-gray-600 mt-1">
                    {{ $partnerType === 'customer' ? 'العميل' : 'المورد' }}: 
                    <span class="font-bold text-gray-800">{{ $ledger['partner']->code }} - {{ $ledger['partner']->name }}</span>
                </p>
            ...
            <table class="w-full text-right border-collapse">
                <thead class="bg-gray-100 border-b border-gray-300 text-xs font-semibold text-gray-600">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">الحساب المقابل</th>
                        <th class="px-4 py-3 text-right">البيان والتفاصيل</th>
                        <th class="px-4 py-3 text-center w-32">مدين (Debit)</th>
                        <th class="px-4 py-3 text-center w-32">دائن (Credit)</th>
                ...
                <tbody class="divide-y divide-gray-200 font-mono text-sm">
                    ...
                    <tr class="bg-blue-50/20 font-bold text-blue-900">
                        <td colspan="4" class="px-4 py-3 text-left font-sans text-xs uppercase tracking-wider">الرصيد الافتتاحي المُرّحل:</td>
                    ...
                    @forelse($ledger['lines'] as $line)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 font-sans">{{ $line['account_code'] }} - {{ $line['account_name'] }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-10 text-center text-gray-500 font-sans">لا توجد حركات مسجلة للشريك خلال الفترة المحددة.</td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <tfoot class="bg-gray-100 border-t-2 border-gray-300 font-mono font-bold text-base text-gray-900">
                    <tr class="bg-gray-50">
                        <td colspan="4" class="px-4 py-4 text-left font-sans">مجموع حركات الفترة:</td>
                    ...
                    <tr class="bg-blue-50/30 text-blue-900 text-lg">
                        <td colspan="4" class="px-4 py-4 text-left font-sans">الرصيد الختامي للفترة:</td>
    ...
    @else
        <div class="bg-white p-12 rounded-xl border border-gray-100 text-center text-gray-500 font-sans">
            ...
            <p>يرجى اختيار الشريك المالي وتحديد فترة التاريخ لعرض كشف الحساب.</p>
        </div>
    @endif
</div>

<script>
function togglePartnerSelects() {
    const type = document.getElementById('partner_type').value;
    const custWrapper = document.getElementById('customer_select_wrapper');
    const suppWrapper = document.getElementById('supplier_select_wrapper');
    
    const custSelect = document.getElementById('customer_id');
    const suppSelect = document.getElementById('supplier_id');

    if (type === 'customer') {
        custWrapper.classList.remove('hidden');
        suppWrapper.classList.add('hidden');
        custSelect.removeAttribute('disabled');
        suppSelect.setAttribute('disabled', 'disabled');
    } else {
        custWrapper.classList.add('hidden');
        suppWrapper.classList.remove('hidden');
        suppSelect.removeAttribute('disabled');
