...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">سندات الصرف (Payment Vouchers)</h2>
            <p class="text-gray-600 mt-1">عرض وإصدار سندات صرف النقدية والمدفوعات والمصروفات المتنوعة</p>
        </div>
        <a href="{{ route('accounting.vouchers.payment.create') }}" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-all shadow hover:shadow-md flex items-center gap-1.5">
            <i class="fas fa-plus-circle"></i> إنشاء سند صرف جديد
        </a>
    </div>
                    ...
                    @forelse($vouchers as $voucher)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-sans flex items-center justify-center gap-3">
                                <a href="{{ route('accounting.vouchers.payment.show', $voucher->id) }}" class="text-blue-600 hover:text-blue-900 font-medium">عرض</a>
                                <span class="text-gray-300">|</span>
                                <a href="{{ route('accounting.vouchers.payment.print', $voucher->id) }}" target="_blank" class="text-gray-600 hover:text-gray-900 font-medium"><i class="fas fa-print"></i> طباعة</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-gray-500 font-sans">لا توجد سندات صرف مسجلة بعد.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
