...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">سند صرف رقم: {{ $journalEntry->entry_number }}</h2>
            <p class="text-gray-600 mt-1">تاريخ الإصدار: {{ $journalEntry->entry_date->toDateString() }}</p>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('accounting.vouchers.payment.print', $journalEntry->id) }}" target="_blank"
            ...
            <a href="{{ route('accounting.vouchers.payment.index') }}" 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-gray-100">
            <div>
                <span class="text-xs text-gray-400 block">المبلغ المصروف</span>
                <span class="font-mono text-2xl font-bold text-red-600 mt-1 block">
